...
SegmentMan::SegmentMan(const std::shared_ptr<DownloadContext>& downloadContext,
    ...
    : downloadContext_(downloadContext),
      ...
      pieceBlockTracker_(make_unique<PieceBlockTracker>())
{
...
SegmentMan::checkoutSegment(cuid_t cuid, const std::shared_ptr<Piece>& piece)
{
  ...
  // Initialize block tracker for this piece if needed
  pieceBlockTracker_->add_piece(piece->getIndex(), piece->countBlock());


  // Subpart 2.3: Urgency Increment (Active Segments)
  if (segment->getLength() > 0) {
    size_t pidx = segment->getIndex();
    std::lock_guard<std::mutex> lock(urgency_mutex_);
    uint16_t newCount = ++urgency_map_[pidx];
    update_heap(pidx, newCount);
  }
...
{
  pieceBlockTracker_->clear_cuid(segment->getIndex(), cuid);
...
void SegmentMan::cancelSegment(cuid_t cuid)
{
  for (auto itr = usedSegmentEntries_.begin(), eoi = usedSegmentEntries_.end();
       itr != eoi;) {
    if ((*itr)->cuid == cuid) {
      // Subpart 2.3: Urgency Decrement (Cancellation)
      auto seg = (*itr)->segment;
      if (seg && seg->getLength() > 0) {
        size_t pidx = seg->getIndex();
        std::lock_guard<std::mutex> lock(urgency_mutex_);
        auto it = urgency_map_.find(pidx);
        if (it != urgency_map_.end() && it->second > 0) {
          uint16_t newCount = --it->second;
          update_heap(pidx, newCount);
        }
      }
                                 ...
                                 const std::shared_ptr<Segment>& segment)
{
  ...
  if (itr == usedSegmentEntries_.end()) {
    ...
    // Subpart 2.3: Urgency Decrement (Completion)
    if (segment->getLength() > 0) {
      size_t pidx = segment->getIndex();
      std::lock_guard<std::mutex> lock(urgency_mutex_);
      auto it = urgency_map_.find(pidx);
...
void SegmentMan::update_heap(size_t index, uint16_t new_count)
{
  // Note: Caller must hold urgency_mutex_
  auto it = urgency_map_.find(index);
  if (it != urgency_map_.end()) {
    urgency_heap_.erase({it->second, index});
  }
  if (new_count > 0) {
    urgency_heap_.insert({new_count, index});
    urgency_map_[index] = new_count;
  }
  else {
    urgency_heap_.erase({new_count + 1, index}); // Ensure deleted (it->second was 1)
    urgency_map_.erase(index);
  }
}

std::vector<size_t> SegmentMan::getUrgentPieceCandidates(uint16_t max_segs)
{
  std::lock_guard<std::mutex> lock(urgency_mutex_);
  std::vector<size_t> candidates;
  for (auto it = urgency_heap_.begin();
       it != urgency_heap_.end() && candidates.size() < 5; ++it) {
    if (it->first < max_segs) {
...
uint16_t SegmentMan::getPieceRequestedCount(size_t index) const
{
  std::lock_guard<std::mutex> lock(urgency_mutex_);
  auto it = urgency_map_.find(index);
  if (it != urgency_map_.end()) {
