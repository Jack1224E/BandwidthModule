...
{
  // Trigger Logic
  size_t unstarted_pieces = bitfieldMan_->countMissingBlock() - usedPieces_.size();
  RequestGroup* group = downloadContext_->getOwnerRequestGroup();
  if (group && !isEndGame()) {
    static int counter = 0;
    if (counter++ % 10 == 0) {
      std::cerr << "DEBUG: unstarted=" << unstarted_pieces 
                << " threshold=" << group->getNumConcurrentCommand() << std::endl;
    }
  }
  if (group && !isEndGame() &&
      unstarted_pieces <= group->getNumConcurrentCommand()) {
    enterEndGame();
    A2_LOG_NOTICE("Entering HTTP End-Game Sniping Mode...");
    std::cerr << "NOTICE: Entering HTTP End-Game Sniping Mode..." << std::endl;
  }

  // Sniper Allocation
  if (isEndGame()) {
    std::vector<size_t> candidates;
    if (bitfieldMan_->getAllMissingIndexes(candidates, ignoreBitfield, length)) {
      std::vector<size_t> inFlight;
      auto segman = group ? group->getSegmentMan() : nullptr;
      for (size_t idx : candidates) {
        if (isPieceUsed(idx)) {
          if (segman && segman->getPieceRequestedCount(idx) >= max_segs_per_piece_) {
            continue;
          }
      ...
      if (!inFlight.empty()) {
        std::shuffle(inFlight.begin(), inFlight.end(),
                     *SimpleRandomizer::getInstance());
        return checkOutPiece(inFlight[0], cuid);
      }
    }
  }
