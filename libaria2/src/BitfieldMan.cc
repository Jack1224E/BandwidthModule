...
bool BitfieldMan::getAllMissingIndexes(std::vector<size_t>& out,
                                       const unsigned char* bitfield,
                                       size_t len) const
{
  size_t mislen = getBitfieldLength();
  auto misbitfield = make_unique<unsigned char[]>(mislen);
  if (!getAllMissingIndexes(misbitfield.get(), mislen, bitfield, len)) {
    return false;
  }
  for (size_t i = 0; i < blocks_; ++i) {
    if (bitfield::test(misbitfield.get(), blocks_, i)) {
      out.push_back(i);
    }
  }
  return !out.empty();
