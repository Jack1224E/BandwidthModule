...
private:
  ...
  std::set<std::pair<uint16_t, size_t>, std::greater<>> urgency_heap_;
  std::unordered_map<size_t, uint16_t> urgency_map_;
  mutable std::mutex urgency_mutex_;

  void update_heap(size_t index, uint16_t new_count);

  std::unique_ptr<PieceBlockTracker> pieceBlockTracker_;
...
public:
  ...
  PieceBlockTracker* getPieceBlockTracker() const
  {
    return pieceBlockTracker_.get();
  }
  ...
  std::vector<size_t> getUrgentPieceCandidates(uint16_t max_segs);

  uint16_t getPieceRequestedCount(size_t index) const;
};
