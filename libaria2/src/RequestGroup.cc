...
void RequestGroup::initPieceStorage()
{
  ...
  if (auto dps = dynamic_cast<DefaultPieceStorage*>(pieceStorage_.get())) {
    A2_LOG_DEBUG("Using UrgencyAwarePieceSelector");
    uint16_t urgency_max_segs =
        (downloadContext_->getPieceLength() +
         option_->getAsInt(PREF_MIN_SPLIT_SIZE) - 1) /
        option_->getAsInt(PREF_MIN_SPLIT_SIZE);
    dps->setPieceSelector(make_unique<UrgencyAwarePieceSelector>(
        segmentMan_.get(), make_unique<LongestSequencePieceSelector>(),
        urgency_max_segs));
  }
